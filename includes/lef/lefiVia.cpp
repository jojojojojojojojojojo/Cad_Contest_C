...
void lefiViaLayer::Init() {
  ...
  this->numRects_ = 0;
  this->rectColorMask_ = (int*)lefMalloc(sizeof(int)*2);
  this->polyColorMask_ = (int*)lefMalloc(sizeof(int)*2);
...
void lefiViaLayer::Destroy() {
  ...
  if (this->polygons_) {
    ...
    for (int i = 0; i < this->numPolys_; i++) {
  ...
  lefFree(this->name_);
  lefFree((char*)(this->rectColorMask_));
  lefFree((char*)(this->polyColorMask_));
}
...
void lefiViaLayer::addRect(int    colorMask,
  ...
  if (this->numRects_ == this->rectsAllocated_) {
    ...
    rectMask = (int*)lefMalloc(sizeof(int)* this->rectsAllocated_);
    ...
    for (i = 0; i < this->numRects_; i++) {
      ...
      newxh[i] = this->xh_[i];
      newyh[i] = this->yh_[i];
      rectMask[i] = this->rectColorMask_[i];
    }
  ...
  this->rectColorMask_[this->numRects_] = colorMask;
 
  this->numRects_ += 1;
}

void lefiViaLayer::addPoly(int   colorMask, 
                           lefiGeometries* geom) {
  if (this->numPolys_ == this->polysAllocated_) {
    ...
    polyMask = (int*)lefMalloc(sizeof(int)* this->polysAllocated_);

    for (i = 0; i < this->numPolys_; i++) {
      poly[i] = this->polygons_[i];
      polyMask[i] = this->polyColorMask_[i];
    }
    ...
    this->polygons_ = poly;
    this->polyColorMask_ = polyMask;
  }

  this->polygons_[this->numPolys_] = geom->lefiGeometries::getPolygon(0);
  this->polyColorMask_[this->numPolys_] = colorMask;
...
lefiViaLayer* lefiViaLayer::clone() {
  ...
  if (this->numRects_ > 0) {
     ...
     vl->rectColorMask_ = (int*)lefMalloc(sizeof(int)* this->numRects_);
     ...
     for (i = 0; i < this->numRects_; i++) {
       ...
       vl->xh_[i] = this->xh_[i];
       vl->yh_[i] = this->yh_[i];
       vl->rectColorMask_[i] = this->rectColorMask_[i];
     }
  } else {
     vl->xl_ = 0;
     vl->yl_ = 0;
  ...
  if (this->numPolys_ > 0) {
    vl->polyColorMask_ = (int*)lefMalloc(sizeof(int)* this->numPolys_);
  } else {
    vl->polyColorMask_ = 0;
  }
  ...
  for (i = 0; i < this->numPolys_; i++) {
    ...
    vl->polygons_[i]->y = (double*)lefMalloc(sizeof(double)*
                          this->polygons_[i]->numPoints);
    vl->polygons_[i]->colorMask = this->polygons_[i]->colorMask;
    vl->polyColorMask_[i] = this->polyColorMask_[i];
...
int lefiViaLayer::rectColorMask(int index) {
  ...
  return this->rectColorMask_[index];
}
...
int lefiViaLayer::polyColorMask(int index) {
  ...
  return this->polyColorMask_[index];
}
...
void lefiVia::addRectToLayer(int mask,
	                     double xl, double yl, 
			     double xh, double yh) {
  this->layers_[this->numLayers_-1]->lefiViaLayer::addRect(mask, xl, yl, xh, yh);
}

void lefiVia::addPolyToLayer(int mask, 
                             lefiGeometries* geom) {
  this->layers_[this->numLayers_-1]->lefiViaLayer::addPoly(mask, geom);
}
...
int lefiVia::rectColorMask(int layerNum, int rectNum) const {
  ...
  return vl->lefiViaLayer::rectColorMask(rectNum);
}
...
int lefiVia::polyColorMask(int layerNum, int polyNum) const {
  ...
  return vl->lefiViaLayer::polyColorMask(polyNum);
}
...
void lefiVia::print(FILE* f) const {
  ...
  if (this->lefiVia::hasProperties()) {
    ...
    for (i = 0; i < this->lefiVia::numLayers(); i++) {
	  ...
	  if (this->lefiVia::rectColorMask(i,h) != 0) {
	       fprintf(f, "    RECT MASK %d %g,%g %g,%g\n",
		       this->lefiVia::rectColorMask(i,h),
