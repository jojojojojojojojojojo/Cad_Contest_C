...
// This file contains code for implementing the defwriter 5.8
// It has all the functions user can call in their callbacks or
// just their writer to write out the correct def syntax.
...
int defwSpecialNetPathPoint(int numPts, double* pointx,
                            double* pointy) {
  int i;
...
int defwSpecialNetPathPointWithWireExt(int numPts, double* pointx,
                     double* pointy, double* optValue) {
  int i;
  defwFunc = DEFW_PATH;   // Current function of writer
  if (! defwFile) return DEFW_UNINITIALIZED;
  if (defwState != DEFW_PATH) return DEFW_BAD_ORDER;
...
int defwSpecialNetShieldPoint(int numPts, double* pointx,
                            double* pointy) {
  int i;
...
int defwNetPathPointWithExt(int numPts, double* pointx, double* pointy,
			    double* optValue) {
  int i;
  defwFunc = DEFW_PATH;   // Current function of writer
  if (! defwFile) return DEFW_UNINITIALIZED;
...
int defwBlockageMask(int colorMask) {
  ...
  if (defwState == DEFW_BLOCKAGE_MASK)
